...
public class TaskFactory {
    ...
    private Task createCompleteGraphTask(User user) {
        int graphVertices = RANDOM.nextInt(
            6-Constants.MIN_COMPLETE_GRAPH_VERTICES
        ...
        if(r==1) {
            ...
            taskType = GraphTaskType.DRAW;
        } else {
            taskType = GraphTaskType.BOOLEAN;
            graphVertices = RANDOM.nextInt(
                Constants.MAX_COMPLETE_GRAPH_VERTICES
                    -Constants.MIN_COMPLETE_GRAPH_VERTICES
            )+Constants.MIN_GRAPH_VERTICES;
        }
    ...
    private Task createHypercubesTask(User user) {
        int graphVertices = Constants.HYPERCUBE_VERTICES[RANDOM.nextInt(
            Constants.HYPERCUBE_VERTICES.length)];
        return Task.builder()
            .uuid(UUID.randomUUID().toString())
            .user(user)
            .graphVertices(graphVertices)
    ...
    private Task createVertexSetSelectionTask(User user,
        ...
        graphEdges -= Math.max(
            RANDOM.nextInt(Constants.MIN_MAX_CLIQUE_VERTICES), graphVertices-1);
        return Task.builder()
            .uuid(UUID.randomUUID().toString())
            .user(user)
            .graphVertices(graphVertices)
    ...
    private Task createMaxIndependentSetTask(User user) {
        return createVertexSetSelectionTask(user,
            GraphTaskSubject.MAX_INDEPENDENT_SET);
    }

    private Task createMinVertexCoverTask(User user) {
        return createVertexSetSelectionTask(user,
            GraphTaskSubject.MIN_VERTEX_COVER);
    }
    ...
    private Task createHamiltonCycleTask(User user) {
        int graphVertices = RANDOM.nextInt(
            Constants.MAX_COMPLETE_GRAPH_VERTICES
                -Constants.MIN_COMPLETE_GRAPH_VERTICES+1
    ...
    private Task createTreeGraphsTask(User user) {
        ...
        GraphTaskType taskType = GraphTaskType.BOOLEAN;
        int r = RANDOM.nextInt(4);
        if(r==1) {
            taskType = GraphTaskType.EDGE_COLORING;
        } else if(r==2) {
    ...
    private Task createIsomorphismTask(User user) {
        int graphVertices = RANDOM.nextInt(
            Constants.MAX_ISOMORPHISM_VERTICES
    ...
    private Task createNamedGraphsTask(User user) {
        String graphName = Constants.GRAPH_NAMES[RANDOM.nextInt(
            Constants.GRAPH_NAMES.length)];
        return Task.builder()
            .uuid(UUID.randomUUID().toString())
            .user(user)
    ...
    private Task createTrivialQuestionTask(User user) {
        String question = Constants.TRIVIAL_QUESTIONS[RANDOM.nextInt(
            Constants.TRIVIAL_QUESTIONS.length)];
        int graphVertices = RANDOM.nextInt(
            Constants.MAX_GRAPH_VERTICES-Constants.MIN_GRAPH_VERTICES
        )+Constants.MIN_GRAPH_VERTICES;
        int graphEdges = RANDOM.nextInt(
    ...
    private Task createDistancesTask(User user) {
        String question = Constants.DISTANCES[RANDOM.nextInt(
            Constants.DISTANCES.length)];
        int graphVertices = RANDOM.nextInt(
        ...
        graphEdges = Math.min(graphEdges,
            (graphVertices*graphVertices-graphVertices)/2);
        int v = -1;
    ...
    private Task createVertexColoringTask(User user) {
        int graphVertices = RANDOM.nextInt(
            Constants.MAX_VERTEX_COLORING_VERTICES
